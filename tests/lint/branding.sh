...
#   usr/share/icons/<brand>  — GENERATED, same reason as the desktoptheme
#                          below. NOTE: this lint reads `git ls-files`, so a
#                          newly generated file is invisible until it is
#                          staged — the icon theme was committed with this
#                          failure latent and it surfaced one commit later.
#   usr/share/plasma/desktoptheme — GENERATED by `just assets`, and its
#                          display name is READ FROM branding.json, which is
#                          exactly what this lint asks for. The staleness
#                          check (tests/lint/theme_generated.py) regenerates
#                          and diffs it, so the name cannot drift from the
#                          source it came from.
...
EXCLUDE_RE='^(os/rootfs/usr/share/meridian/branding\.json|os/rootfs/usr/share/plasma/desktoptheme|os/rootfs/usr/share/icons|docs|STATUS\.md|CONTRIBUTING-AGENTS\.md|README\.md|tests/lint/branding\.sh)(/|$)'
